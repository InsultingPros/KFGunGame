...
function SetupGroups()
{
  ...
  else
  {
    for (i = 0; i < Controls.Length; i++)
    {
      if (Controls[i] == b_Team || Controls[i] == b_Settings || Controls[i] == b_Browser || Controls[i] == b_Quit || Controls[i] == b_Favs || Controls[i] == b_Leave || Controls[i] == b_MapVote || Controls[i] == b_KickVote || Controls[i] == b_MatchSetup || Controls[i] == b_Spec)
      {
...
function SetButtonPositions(Canvas C)
{
  ...
  {
    if (Components[i].bVisible && GUIButton(Components[i]) != none && Components[i] != PrevHintButton && Components[i] != NextHintButton) // Components[i] != b_Team
    {
      NumButtons++;
    }
  }
